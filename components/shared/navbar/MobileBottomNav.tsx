...
import { useSearchModal } from '@/hooks/useSearchModal'; // Import the hook

const MobileBottomNav = () => {
  const [isVisible, setIsVisible] = useState(true);
  const [lastScrollY, setLastScrollY] = useState(0);
  const searchModal = useSearchModal(); // Access the search modal state
  ...
  return (
    <div
      className={`fixed bottom-0 left-0 right-0 z-50 bg-white shadow-md p-3 flex justify-around items-center transition-transform duration-300 ${
        isVisible && !searchModal.isOpen ? 'translate-y-0' : 'translate-y-full'
      } md:hidden`} // Hide if search modal is open
    >
      <Link href='/'>
